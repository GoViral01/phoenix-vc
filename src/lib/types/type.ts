...
export const founderRegistrationSchema = z
  .object({
    first_name: z
      .string()
    ...
    instagram: z
      ...
      .refine(
        (url) =>
          !url
            ? true
            : /(https?:\/\/)?(.*?\.)?instagram\.com\/\w+\/?$/.test(url),
  ...
  .refine(
    (data) => (data.niche === "others" ? Boolean(data.other_niche) : true),
    {
      message: "Enter your niche",
      path: ["other_niche"],
    }
  );
